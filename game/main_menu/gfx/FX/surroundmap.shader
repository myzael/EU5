	...
	MainCode VS_surroundmap
	{
		...
		[[
			...
			{			
				...
				float3 WorldSpacePos = float3( Input.position.x, GetFlatMapHeight(), Input.position.y );
				#ifndef SURROUND_SHADOW
					WorldSpacePos.y += CloudHeight * ( 1.0 - GetFlatMapLerp() );
				#endif
				VertexOut.position = FixProjectionAndMul( ViewProjectionMatrix, float4( WorldSpacePos, 1.0 ) );
				VertexOut.uv = Input.position / GetMapSize();
	...
	MainCode PS_surroundmap
	{
		...
		[[
			...
			PDX_MAIN
			{
				...
				float2 BaseCloudOffset = GetScaledGlobalTime() * BaseCloudScrolling;
				...
				TangentSpaceCameraDir.xz /= GetMapSize();
				...
				float3 Normal2 = CalculateNormal( CloudTexture, (AnimatedBaseCloudUV + GetScaledGlobalTime() * Cloud1Scrolling) * Cloud1TileFactor, Cloud1Strength );
				float3 Normal3 = CalculateNormal( CloudTexture, (AnimatedBaseCloudUV + GetScaledGlobalTime() * Cloud2Scrolling) * Cloud2TileFactor, Cloud2Strength );
				...
				Color = lerp( Color, ZoomedOut.rgb, GetFlatMapLerp() );
				...
				FinalAlpha = lerp( FinalAlpha, ZoomedOut.a, GetFlatMapLerp() ) * Mask;
	...
	MainCode PS_surroundmap_shadow
	{
		...
		[[			
			PDX_MAIN
			{
				...
				return float4( ShadowColor, Mask * ( 1.0 - saturate( GetFlatMapLerp() * 2.0 - 1.0 ) ) );
			}
		]]
	}
