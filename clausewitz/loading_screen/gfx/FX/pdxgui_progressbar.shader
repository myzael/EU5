	...
	MainCode PixelShader
	{
		...
		[[
			PDX_MAIN
			{
				...
				bool ProgressTest = ProgressPos.x <= Progress.x && ProgressPos.y <= Progress.y;
			
				// If/else was used instead of the ternary operator here, but for some reason it caused some flickering on AMD cards
				float4 OutColor = ProgressTest ? SampleImageSprite( Texture0, Input.UV0 ) : SampleImageSprite( Texture1, Input.UV0 );
